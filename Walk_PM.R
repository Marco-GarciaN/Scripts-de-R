...
ant <- read_sas(paste("Y:\\Jorge\\Calificacion_CNBV\\Nueva_Metodología\\Resultados\\",
  ...
  filter(Cartera_cont>0 & TipoP=="PM") %>%
...
Cartera_PM_E1 <- data.frame(Walk_PM_out[Walk_PM_out$clase==1,c("Suma_OS_act", "Sub_clase")]) %>%
  arrange(Suma_OS_act) %>% rename(Etapa_1 = Suma_OS_act) %>% na.omit()
Cartera_PM_E1$Numero <- formatC(Cartera_PM_E1$Etapa_1,format= "f",digits= 2, big.mark = ",")

Cartera_PM_E2 <- data.frame(Walk_PM_out[Walk_PM_out$clase==2,c("Suma_OS_act", "Sub_clase")]) %>%
  arrange(Suma_OS_act) %>% rename(Etapa_2 = Suma_OS_act) %>% na.omit()
Cartera_PM_E2$Numero <- formatC(Cartera_PM_E2$Etapa_2,format= "f",digits= 2, big.mark = ",")

Cartera_PM_E3 <- data.frame(Walk_PM_out[Walk_PM_out$clase==3,c("Suma_OS_act", "Sub_clase")]) %>%
  arrange(Suma_OS_act) %>% rename(Etapa_3 = Suma_OS_act) %>% na.omit()
Cartera_PM_E3$Numero <- formatC(Cartera_PM_E3$Etapa_3,format= "f",digits= 2, big.mark = ",")


Reserva_PM_E1 <- data.frame(Walk_PM_out[Walk_PM_out$clase==1,c("Suma_Rva_act", "Sub_clase")]) %>%
  arrange(Suma_Rva_act) %>% rename(Etapa_1 = Suma_Rva_act) %>% na.omit()
Reserva_PM_E1$Numero <- formatC(Reserva_PM_E1$Etapa_1,format= "f",digits= 2, big.mark = ",")

Reserva_PM_E2 <- data.frame(Walk_PM_out[Walk_PM_out$clase==2,c("Suma_Rva_act", "Sub_clase")]) %>%
  arrange(Suma_Rva_act) %>% rename(Etapa_2 = Suma_Rva_act) %>% na.omit()
Reserva_PM_E2$Numero <- formatC(Reserva_PM_E2$Etapa_2,format= "f",digits= 2, big.mark = ",")

Reserva_PM_E3 <- data.frame(Walk_PM_out[Walk_PM_out$clase==3,c("Suma_Rva_act", "Sub_clase")]) %>%
  arrange(Suma_Rva_act) %>% rename(Etapa_3 = Suma_Rva_act) %>% na.omit()
Reserva_PM_E3$Numero <- formatC(Reserva_PM_E3$Etapa_3,format= "f",digits= 2, big.mark = ",")

rm(ant, act, base)

##################################################################################

PM1 <- ggplot(Cartera_PM_E1, aes(x = Sub_clase, y = Etapa_1)) +
  geom_bar(stat = "identity", color = "green3",lwd = 0.9, 
           fill = "limegreen", alpha=0.7, width = 0.40)+
  scale_x_discrete(limits = Cartera_PM_E1$Sub_clase)+
  ...
  ylim(c(0, max(Cartera_PM_E1$Etapa_1)*1.9))+
  coord_flip()+
  labs(title = "Etapa 1",
       subtitle = formatC(sum(Cartera_PM_E1$Etapa_1),
...
PM2 <- ggplot(Cartera_PM_E2, aes(x = Sub_clase, y = Etapa_2)) +
  geom_bar(stat = "identity", color = "orange2",lwd = 0.9, 
           fill = "orange", alpha=0.7, width = 0.3)+
  scale_x_discrete(limits = Cartera_PM_E2$Sub_clase)+
  geom_text(aes(label = Numero), vjust = 1.1, hjust=-0.1,
            colour = "darkorange2", size=4)+
  geom_text(aes(label = Sub_clase), vjust = -0.4, hjust=-0.1,
            colour = "black", size=3.5)+
  ylim(c(0, max(Cartera_PM_E2$Etapa_2)*1.9))+
  coord_flip()+
  labs(title = "Etapa 2",
       subtitle = formatC(sum(Cartera_PM_E2$Etapa_2),
                          format= "f",digits= 2, big.mark = ","),
       x = "", y = "") + theme_hc() +
  theme(axis.text.x = element_blank(),
        axis.ticks.x = element_blank(),
...
PM3 <- ggplot(Cartera_PM_E3, aes(x = Sub_clase, y = Etapa_3)) +
  geom_bar(stat = "identity", color = "red2",lwd = 0.9, 
           fill = "red", alpha=0.7, width = 0.3)+
  scale_x_discrete(limits = Cartera_PM_E3$Sub_clase)+
  geom_text(aes(label = Numero), vjust = 1.1 , hjust=-0.1,
            colour = "red3", size=4)+
  geom_text(aes(label = Sub_clase), vjust = -0.4 , hjust=-0.1,
            colour = "black", size=3.5)+
  ylim(c(0, max(Cartera_PM_E3$Etapa_3)*1.9))+
  coord_flip()+
  labs(title = "Etapa 3",
       subtitle = formatC(sum(Cartera_PM_E3$Etapa_3),
                          format= "f",digits= 2, big.mark = ","),
       x = "", y = "") + theme_hc() +
  theme(axis.text.x = element_blank(),
        axis.ticks.x = element_blank(),
...
annotate_figure(ggarrange(PM1, PM2, PM3, ncol = 3, nrow = 1), 
                top = text_grob(paste0("Cartera PM ",
...
RvaPM1 <- ggplot(Reserva_PM_E1, aes(x = Sub_clase, y = Etapa_1)) +
  geom_bar(stat = "identity", color = "green3",lwd = 0.9, 
           fill = "limegreen", alpha=0.7, width = 0.40)+
  scale_x_discrete(limits = Reserva_PM_E1$Sub_clase)+
  geom_text(aes(label = Numero), vjust = 1.1 , hjust=-0.1,
            colour = "limegreen", size=4)+
  geom_text(aes(label = Sub_clase), vjust = -0.6 , hjust=-0.1,
            colour = "black", size=3.5)+
  ylim(c(0, max(Reserva_PM_E1$Etapa_1)*1.9))+
  coord_flip()+
  labs(title = "Etapa 1",
       subtitle = formatC(sum(Reserva_PM_E1$Etapa_1),
                          format= "f",digits= 2, big.mark = ","),
       x = "", y = "") + theme_hc() +
  theme(axis.text.x = element_blank(),
        axis.ticks.x = element_blank(),
...
RvaPM2 <- ggplot(Reserva_PM_E2, aes(x = Sub_clase, y = Etapa_2)) +
  geom_bar(stat = "identity", color = "orange2",lwd = 0.9, 
           fill = "orange", alpha=0.7, width = 0.3)+
  scale_x_discrete(limits = Reserva_PM_E2$Sub_clase)+
  geom_text(aes(label = Numero), vjust = 1.1, hjust=-0.1,
            colour = "darkorange2", size=4)+
  geom_text(aes(label = Sub_clase), vjust = -0.4, hjust=-0.1,
            colour = "black", size=3.5)+
  ylim(c(0, max(Reserva_PM_E2$Etapa_2)*1.9))+
  coord_flip()+
  labs(title = "Etapa 2",
       subtitle = formatC(sum(Reserva_PM_E2$Etapa_2),
                          format= "f",digits= 2, big.mark = ","),
       x = "", y = "") + theme_hc() +
  theme(axis.text.x = element_blank(),
        axis.ticks.x = element_blank(),
...
RvaPM3 <- ggplot(Reserva_PM_E3, aes(x = Sub_clase, y = Etapa_3)) +
  geom_bar(stat = "identity", color = "red2",lwd = 0.9, 
           fill = "red", alpha=0.7, width = 0.3)+
  scale_x_discrete(limits = Reserva_PM_E3$Sub_clase)+
  geom_text(aes(label = Numero), vjust = 1.1 , hjust=-0.1,
            colour = "red3", size=4)+
  geom_text(aes(label = Sub_clase), vjust = -0.4 , hjust=-0.1,
            colour = "black", size=3.5)+
  ylim(c(0, max(Reserva_PM_E3$Etapa_3)*1.9))+
  coord_flip()+
  labs(title = "Etapa 3",
       subtitle = formatC(sum(Reserva_PM_E3$Etapa_3),
                          format= "f",digits= 2, big.mark = ","),
       x = "", y = "") + theme_hc() +
  theme(axis.text.x = element_blank(),
        axis.ticks.x = element_blank(),
...
annotate_figure(ggarrange(RvaPM1, RvaPM2, RvaPM3, ncol = 3, nrow = 1), 
                top = text_grob(paste0("Reserva PM ",
                                       toTitleCase(as.character(month(fecha_ant,label=TRUE,abbr=TRUE))),"-",
                                       toTitleCase(as.character(month(fecha_act,label=TRUE,abbr=TRUE))), 
                                       " ", year(fecha_act)),      
                                color = "black", face = "bold", size = 22))
